...
export const AuditKpiCard: React.FC<AuditKpiCardProps> = ({ title, value, subtitle, trend, icon, accent }) => {
  return (
    <div className={`rounded-xl border bg-gradient-to-br from-navy-900 to-navy-950 p-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_24px_rgba(15,23,42,0.7)] ${accentClasses[accent]}`}>
      <div className="flex items-start justify-between">
        ...
        {trend ? <span className="text-[11px] px-2 py-1 rounded-md bg-navy-800 text-slate-300">{trend}</span> : null}
      </div>
