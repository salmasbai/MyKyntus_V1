...
import { WORKFLOW_ACTIONS, type AdminWorkflowConfig } from '../../mock-data/admin';
...
interface Props {
  ...
  onChange: (next: AdminWorkflowConfig) => void;
          ...
          <div className="relative card-navy max-w-xl w-full p-6 border border-navy-800">
            {mode === 'config' ? (
              <div className="space-y-4">
                <h3 className="text-white font-semibold text-lg">Modifier: {selectedStep.role}</h3>
                <label className="text-slate-300 text-sm">Rôle
                  <input readOnly value={selectedStep.role} className="mt-1 w-full bg-navy-800 border border-navy-700 rounded-lg p-3 text-sm text-slate-200" />
                </label>
                ...
                <label className="text-slate-300 text-sm">Type notification
                  <select value={selectedStep.notificationType} onChange={(e) => onChange({ ...workflow, steps: workflow.steps.map((s, i) => i === stepIndex ? { ...s, notificationType: e.target.value as 'email' | 'in-app' } : s) })} className="mt-1 w-full bg-navy-800 border border-navy-700 rounded-lg p-3 text-sm text-slate-200">
              ...
              <div className="space-y-3">
                <h3 className="text-white font-semibold text-lg">Détails: {selectedStep.role}</h3>
                <p className="text-slate-300 text-sm">{roleDetails[selectedStep.role]?.description ?? 'Étape de validation du workflow.'}</p>
